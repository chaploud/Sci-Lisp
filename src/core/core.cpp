 ...
 public:
  ...
  Value(std::vector<Value> params, Value ret, Environment const &env) : type(LAMBDA) {
    ...
    for (size_t i = 0; i < used_atoms.size(); i++) {
      ...
      if (env.has(used_atoms[i])) lambda_scope.set(used_atoms[i], env.get(used_atoms[i]));
    }
  }
  ...
  double as_float() const { return cast_to_int().stack_data.f; }

  // Get this item's string value
  std::string as_string() const {
    // If this item is not a string, throw a cast error.
    if (type != STRING) throw Error(*this, Environment(), BAD_CAST);
    return str;
  }
  ...
  std::string as_atom() const {
    ...
    if (type != ATOM) throw Error(*this, Environment(), BAD_CAST);
    return str;
  }
  ...
  std::vector<Value> as_list() const {
    ...
    if (type != LIST) throw Error(*this, Environment(), BAD_CAST);
    return list;
  }
  ...
  void push(Value val) {
    ...
    // Throw an error.
    if (type != LIST) throw Error(*this, Environment(), MISMATCHED_TYPES);
  ...
  Value cast_to_float() const {
    switch (type) {
      case FLOAT:
        return *this;
      case INT:
        return Value(float(stack_data.i));
  ...
  bool operator<(Value other) const {
    ...
    if (other.type != FLOAT && other.type != INT) throw Error(*this, Environment(), INVALID_BIN_OP);
    ...
    switch (type) {
      ...
      case INT:
        ...
        if (other.type == FLOAT) return cast_to_float().stack_data.f < other.stack_data.f;
  ...
  Value operator+(Value other) const {
    ...
    switch (type) {
      ...
      case INT:
        ...
        if (other.type == FLOAT) return Value(cast_to_float() + other.stack_data.f);
      ...
      case STRING:
        ...
        if (other.type == STRING) return Value::string(str + other.str);
      ...
      case LIST:
        ...
        if (other.type == LIST) {
          ...
          for (size_t i = 0; i < other.list.size(); i++) result.push(other.list[i]);
          return result;
  ...
  Value operator-(Value other) const {
    ...
    switch (type) {
      ...
      case INT:
        ...
        if (other.type == FLOAT) return Value(cast_to_float().stack_data.f - other.stack_data.f);
...
// If we support libm, we can find the remainder of floating point values.
      ...
      case INT:
        ...
        if (other.type != INT) throw Error(other, Environment(), NO_LIBM_SUPPORT);
        return Value(stack_data.i % other.stack_data.i);
#endif
  ...
  friend std::ostream &operator<<(std::ostream &os, Value const &v) { return os << v.display(); }

 private:
  enum { QUOTE, ATOM, INT, FLOAT, LIST, STRING, LAMBDA, BUILTIN, UNIT } type;
...
Error::~Error() { delete cause; }

std::string Error::description() {
  return "error: the expression `" + cause->debug() + "` failed in scope " + to_string(env) +
         " with message \"" + msg + "\"";
}
...
void Environment::set(std::string name, Value value) { defs[name] = value; }
  ...
  switch (type) {
    case LAMBDA:
      ...
      if (params.size() != args.size())
        throw Error(Value(args), env, args.size() > params.size() ? TOO_MANY_ARGS : TOO_FEW_ARGS);
      ...
      for (size_t i = 0; i < params.size(); i++) {
        if (params[i].type != ATOM) throw Error(*this, env, INVALID_LAMBDA);
...
Value Value::eval(Environment &env) {
  ...
  switch (type) {
    ...
    case ATOM:
      return env.get(str);
    case LIST:
      if (list.size() < 1) throw Error(*this, env, EVAL_EMPTY_LIST);
      ...
      if (!function.is_builtin())
        for (size_t i = 0; i < args.size(); i++) args[i] = args[i].eval(env);
...
std::vector<Value> parse(std::string s) {
  ...
  if (i < int(s.length())) throw std::runtime_error(MALFORMED_PROGRAM);
...
Value run(std::string code, Environment &env) {
  ...
  // in this environment.
  for (size_t i = 0; i < parsed.size() - 1; i++) parsed[i].eval(env);
...
Value lambda(std::vector<Value> args, Environment &env) {
  if (args.size() < 2) throw Error(Value("lambda", lambda), env, TOO_FEW_ARGS);
...
Value defun(std::vector<Value> args, Environment &env) {
  ...
  if (args[1].get_type_name() != LIST_TYPE) throw Error(Value("defun", defun), env, INVALID_LAMBDA);
...
Value while_loop(std::vector<Value> args, Environment &env) {
  ...
  while (args[0].eval(env).as_bool()) {
    for (size_t i = 1; i < args.size() - 1; i++) args[i].eval(env);
...
Value for_loop(std::vector<Value> args, Environment &env) {
  ...
  for (size_t i = 0; i < list.size(); i++) {
    ...
    for (size_t j = 1; j < args.size() - 1; j++) args[j].eval(env);
    acc = args[args.size() - 1].eval(env);
  }
...
Value do_block(std::vector<Value> args, Environment &env) {
  Value acc;
  for (size_t i = 0; i < args.size(); i++) acc = args[i].eval(env);
  return acc;
}
...
Value scope(std::vector<Value> args, Environment &env) {
  Environment e = env;
  Value acc;
  for (size_t i = 0; i < args.size(); i++) acc = args[i].eval(e);
  return acc;
}
...
Value quote(std::vector<Value> args, Environment &) {
  std::vector<Value> v;
  for (size_t i = 0; i < args.size(); i++) v.push_back(args[i]);
  return Value(v);
}
...
Value print(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() < 1) throw Error(Value("print", print), env, TOO_FEW_ARGS);
...
Value input(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() > 1) throw Error(Value("input", input), env, TOO_MANY_ARGS);
...
Value write_file(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() != 2)
    throw Error(Value("write-file", write_file), env,
...
Value sum(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() < 2) throw Error(Value("+", sum), env, TOO_FEW_ARGS);

  Value acc = args[0];
  for (size_t i = 1; i < args.size(); i++) acc = acc + args[i];
  return acc;
}
...
Value product(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() < 2) throw Error(Value("*", product), env, TOO_FEW_ARGS);
...
Value cast_to_float(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() != 1)
    throw Error(Value(FLOAT_TYPE, cast_to_float), env,
                args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value index(std::vector<Value> args, Environment &env) {
  ...
  int i = args[1].as_int();
  if (list.empty() || i >= (int)list.size()) throw Error(list, env, INDEX_OUT_OF_RANGE);
...
Value insert(std::vector<Value> args, Environment &env) {
  ...
  std::vector<Value> list = args[0].as_list();
  int i = args[1].as_int();
  if (i > (int)list.size()) throw Error(list, env, INDEX_OUT_OF_RANGE);
...
Value len(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() != 1)
    throw Error(Value("len", len), env, args.size() > 1 ? TOO_MANY_ARGS : TOO_FEW_ARGS);
...
Value push(std::vector<Value> args, Environment &env) {
  ...
  if (args.size() == 0) throw Error(Value("push", push), env, TOO_FEW_ARGS);
  for (size_t i = 1; i < args.size(); i++) args[0].push(args[i]);
  return args[0];
}
...
Value head(std::vector<Value> args, Environment &env) {
  ...
  std::vector<Value> list = args[0].as_list();
  if (list.empty()) throw Error(Value("head", head), env, INDEX_OUT_OF_RANGE);
...
Value tail(std::vector<Value> args, Environment &env) {
  ...
  for (size_t i = 1; i < list.size(); i++) result.push_back(list[i]);
...
Value parse(std::vector<Value> args, Environment &env) {
  ...
  if (args[0].get_type_name() != STRING_TYPE) throw Error(args[0], env, INVALID_ARGUMENT);
...
Value filter_list(std::vector<Value> args, Environment &env) {
  ...
  for (size_t i = 0; i < l.size(); i++) {
    tmp.push_back(l[i]);
    if (args[0].apply(tmp, env).as_bool()) result.push_back(l[i]);
    tmp.clear();
  }
  return Value(result);
}
...
int main(int argc, const char **argv) {
  Environment env;
  std::vector<Value> args;
  for (int i = 0; i < argc; i++) args.push_back(Value::string(argv[i]));
...
#else
  if (argc == 3 && std::string(argv[1]) == "-c") run(argv[2], env);
#endif
