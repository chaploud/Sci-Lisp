...
impl PartialEq for Value {
    fn eq(&self, other: &Value) -> bool {
        match (self, other) {
            ...
            (ConstMacro, ConstMacro) => true,
            (SetMacro, SetMacro) => true,
            (SliceMacro, SliceMacro) => true,
            (LetMacro, LetMacro) => true,
            (DoMacro, DoMacro) => true,
            (FnMacro, FnMacro) => true,
            (IfMacro, IfMacro) => true,
            (SwitchMacro, SwitchMacro) => true,
            (ForMacro, ForMacro) => true,
            (WhileMacro, WhileMacro) => true,
            (BreakMacro, BreakMacro) => true,
            (ContinueMacro, ContinueMacro) => true,
            (EnumMacro, EnumMacro) => true,
            (StructMacro, StructMacro) => true,
            (MacroMacro, MacroMacro) => true,
            (ClassMacro, ClassMacro) => true,
            (ThreadMacro, ThreadMacro) => true,
            _ => false,
        }
    }
}
...
impl Hash for Value {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        match self {
            ...
            QuoteMacro => Value::QuoteMacro.hash(state),
            DefMacro => Value::DefMacro.hash(state),
            ConstMacro => Value::ConstMacro.hash(state),
            SetMacro => Value::SetMacro.hash(state),
            SliceMacro => Value::SliceMacro.hash(state),
            LetMacro => Value::LetMacro.hash(state),
            DoMacro => Value::DoMacro.hash(state),
            FnMacro => Value::FnMacro.hash(state),
            IfMacro => Value::IfMacro.hash(state),
            SwitchMacro => Value::SwitchMacro.hash(state),
            ForMacro => Value::ForMacro.hash(state),
            WhileMacro => Value::WhileMacro.hash(state),
            BreakMacro => Value::BreakMacro.hash(state),
            ContinueMacro => Value::ContinueMacro.hash(state),
            EnumMacro => Value::EnumMacro.hash(state),
            StructMacro => Value::StructMacro.hash(state),
            MacroMacro => Value::MacroMacro.hash(state),
            ClassMacro => Value::ClassMacro.hash(state),
            ThreadMacro => Value::ThreadMacro.hash(state),
        }
    }
}
...
impl fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            FnMacro => write!(f, "macro 'fn'"),
            IfMacro => write!(f, "macro 'if'"),
            SwitchMacro => write!(f, "macro 'switch'"),
            ForMacro => write!(f, "macro 'for'"),
            WhileMacro => write!(f, "macro 'while'"),
            BreakMacro => write!(f, "macro 'break'"),
            ContinueMacro => write!(f, "macro 'continue'"),
            EnumMacro => write!(f, "macro 'enum'"),
            StructMacro => write!(f, "macro 'struct'"),
            MacroMacro => write!(f, "macro 'macro'"),
            ClassMacro => write!(f, "macro 'class'"),
            ThreadMacro => write!(f, "macro '->'"),
        }
    }
}
...
impl Value {
    pub fn type_name(&self) -> Result<Value> {
        let result = match self {
            ...
            Value::QuoteMacro => TypeName::Macro.to_string(),
            Value::DefMacro => TypeName::Macro.to_string(),
            Value::ConstMacro => TypeName::Macro.to_string(),
            Value::SetMacro => TypeName::Macro.to_string(),
            Value::SliceMacro => TypeName::Macro.to_string(),
            Value::LetMacro => TypeName::Macro.to_string(),
            Value::DoMacro => TypeName::Macro.to_string(),
            Value::FnMacro => TypeName::Macro.to_string(),
            Value::IfMacro => TypeName::Macro.to_string(),
            Value::SwitchMacro => TypeName::Macro.to_string(),
            Value::ForMacro => TypeName::Macro.to_string(),
            Value::WhileMacro => TypeName::Macro.to_string(),
            Value::BreakMacro => TypeName::Macro.to_string(),
            Value::ContinueMacro => TypeName::Macro.to_string(),
            Value::EnumMacro => TypeName::Macro.to_string(),
            Value::StructMacro => TypeName::Macro.to_string(),
            Value::MacroMacro => TypeName::Macro.to_string(),
            Value::ClassMacro => TypeName::Macro.to_string(),
            Value::ThreadMacro => TypeName::Macro.to_string(),
        };
